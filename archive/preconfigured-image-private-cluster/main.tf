...
resource "google_compute_address" "workstation_cluster_ip" {
  project                = module.project.project_id
  name                   = "workstations-cluster-ip"
...
resource "google_compute_forwarding_rule" "workstation_psc" {
  project                 = module.project.project_id
  name                    = "workstations-cluster-psc"
  region                  = var.region
  load_balancing_scheme   = ""
  target                  = google_workstations_workstation_cluster.cluster.private_cluster_config[0].service_attachment_uri
  network                 = local.network_id
  ip_address              = google_compute_address.workstation_cluster_ip.id
  allow_psc_global_access = true

  depends_on = [google_workstations_workstation_cluster.cluster]
}

resource "google_dns_managed_zone" "workstation_private_zone" {
  project              = module.project.project_id
  name                 = "workstation-private-zone"
  dns_name             = "${google_workstations_workstation_cluster.cluster.private_cluster_config[0].cluster_hostname}."
  description          = "Private DNS Zone for Cloud Workstations"
  labels = {
    cloud-workstations = "true"
  }
...
resource "google_dns_record_set" "workstation_a" {
  project      = module.project.project_id
  name         = "*.${google_workstations_workstation_cluster.cluster.private_cluster_config[0].cluster_hostname}."
  managed_zone = google_dns_managed_zone.workstation_private_zone.name
  type         = "A"
  ttl          = 300

  rrdatas = ["${google_compute_forwarding_rule.workstation_psc.ip_address}"]
}
...
#   project               = module.project.project_id
#   name                  = "workstations-psc-neg"
#   region                = var.region

#   network_endpoint_type = "PRIVATE_SERVICE_CONNECT"
#   psc_target_service    = google_workstations_workstation_cluster.cluster.private_cluster_config[0].service_attachment_uri
...
#   project               = module.project.project_id
#   name                  = "workstations-backend-service"
